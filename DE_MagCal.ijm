...
if (activeImageTitle == "MagCal_Au_PS") {

	getLine(lineX1, lineY1, lineX2, lineY2, lineWidth);
	if (lineX1 < 0) {
		print("Error");
		exit("You must draw a line specifying the approximate diamater of the 2.36 A gold ring in the FFT.");
	}
	lineHalfLength = sqrt((lineX2 - lineX1) * (lineX2 - lineX1) + (lineY2 - lineY1) * (lineY2 - lineY1)) / 2;
	...
	lineHalfLength = lineHalfLength / ((plotX[plotX.length - 1] - plotX[0]) / plotX.length);
	
	print("Finding gold peak near the specified diameter...");

	bgSmoothingRadius = Math.round(psWidth / 32.0);
	
	bgSubtractedY = newArray(plotX.length);
	for (n = 0; n < plotX.length; n++) {
		if ((n < maxOf(bgSmoothingRadius, lineHalfLength - bgSmoothingRadius)) || (n > (lineHalfLength + bgSmoothingRadius))) {
			bgSubtractedY[n] = 0.0;
		}
		else {
			bgMinX = n - bgSmoothingRadius;
			if (bgMinX < 0) {
				bgMinX = 0;
			}
			bgMaxX = n + bgSmoothingRadius;
			if (bgMaxX >= plotX.length) {
				bgMaxX = plotX.length - 1;
			}
			bgArrayX = newArray(bgMaxX - bgMinX + 1);
			for (i = bgMinX; i <= bgMaxX; i++) {
				bgArrayX[i - bgMinX] = i;
			}
			bgArrayY = Array.slice(plotY, bgMinX, bgMaxX + 1);
			Fit.doFit("Straight Line", bgArrayX, bgArrayY);
			bgSubtractedY[n] = plotY[n] - (Fit.p(0) + Fit.p(1) * n);
		}
	}
	
	maxValue = 0.0;
	maxLocation = 0;
	for (n = 1; n < (plotX.length - 1); n++) {
...
if (typeOfData == 1) {
	print("");
	print("If the ring on the FFT does not match the 2.36 A gold FFT ring, please draw a line ");
	print("across the diameter of the correct FFT ring and then re-run this macro.");
}
